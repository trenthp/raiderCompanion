...
async function createFirebaseExport(items) {
  ...
  const firebaseItems = items.map(convertItemToFirebase);
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(itemsExport, null, 2)
  );

  console.log(`✓ Created ${firebaseItems.length} items in Firebase emulator format`);
}
